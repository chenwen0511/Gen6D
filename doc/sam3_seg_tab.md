...
RGB + 传感器深度 + camera.json
        ...
        │         │
        │         ▼
        │   孔洞同排连线 PCA 定水平 X + 各孔 ABCD/abcd(旋45°) 8 点联合拟合法向 Z
        │   + LED 圆心定中心 / LED 8 点定深度 ──► P1
        │
        └─► 各实例点云（传感器深度反投影）
                  │
                  ▼
...
- 位置 `x,y,z`：**相机系 `frame=camera`**，单位 **mm**（X 右、Y 下、Z 前）
- 姿态 `rx,ry,rz`：单位 **°**，由旋转矩阵按 **ZYX** 欧拉角换算后写成 `[rx,ry,rz]`（Roll / Pitch / Yaw）
- 3D 点云预览：`preview_frame=glb_y_up`（点云与 P1/q 坐标轴均做 **Y 翻转** 后显示；与 JSON 数值同一几何，仅显示系不同）
...
| 标记位 P1 | 孔洞 / LED / 外接正方形 ABCD / 坐标轴 |
| ABCD 放大 | LED 外接正方形 ABCD + 旋转 45° 的 abcd（共 8 角点 / 深度） |
...
| 符号 | 含义 |
|------|------|
| **P1** | 货架面板蓝色 LED 标记位中心位姿（各孔 8 点联合定面板朝向 Z，孔排定水平 X，LED 定中心/深度） |
