...
def fuse_sensor_and_estimated(
    ...
    if not align.success:
        fused = np.where(valid_mask, sensor_depth, np.nan).astype(np.float32)
        return FusionResult(
            fused_depth=fused,
            metric_est_depth=apply_scale_shift(est_resized, 1.0, 0.0),
            valid_mask=valid_mask,
            inpaint_mask=~valid_mask & np.isfinite(fused),
            scale=align.scale,
            shift=align.shift,
            align_inlier_ratio=align.inlier_ratio,
            status="fallback_sensor_only",
            message="对齐失败，仅保留传感器有效区域",
        )
