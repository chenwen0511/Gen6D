...
def create_app(model_dir: str, device: str) -> FastAPI:
    ...
    async def _infer_grasp_handler(
        ...
        try:
            result = infer_grasp_from_uploads(
                ...
                hole_prompt=hole_prompt or DEFAULT_PLACE_HOLE_PROMPT,
    ...
    async def infer_grasp_v1(
        ...
        marker_prompt: Annotated[
            str | None, Form(description="兼容旧字段，等同 led_prompt（蓝色 LED）")
        ] = None,
        ...
        y_band_mm: Annotated[
            float, Form(description="球筛后相机 y ± 带宽（mm），默认 2.0")
        ] = 2.0,
    ):
    ...
    async def infer_grasp_alias(
        ...
        hole_prompt: Annotated[str | None, Form()] = None,
